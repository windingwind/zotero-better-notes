...
class EditorViews extends AddonBase {
  ...
  public getEditorElement(_document: Document): HTMLElement {
  ...
  public async updateEditorLinkPopup(
    ...
    if (note) {
      const insertButtonId = "link-popup-insert";
      let insertButton = _window.document.getElementById(insertButtonId);
      if (insertButton) {
        insertButton.remove();
      }
      ...
      insertButton.innerHTML = this.icons["embedLinkContent"];
      ...
      const updateButtonId = "link-popup-update";
      let updateButton = _window.document.getElementById(updateButtonId);
      if (updateButton) {
        updateButton.remove();
      }
      ...
      const openInWindowButtonId = "link-popup-open-window";
      let openInWindowButton =
        _window.document.getElementById(openInWindowButtonId);
  ...
  public updatePopupMenu() {
    ...
    const lineElements = this._Addon.NoteParse.parseHTMLElements(
      this.getEditorElement(
        this._Addon.EditorController.activeEditor._iframeWindow.document
      ) as HTMLElement
    ) as HTMLElement[];
    const charLength = (c) => {
      if (/^[\u0000-\u00ffA-Za-z1-9]+$/.test(c.charAt(0))) {
        return 1;
      } else {
        return 2;
      }
    };
    const shorten = (str: string, threshold: number) => {
      let res = "";
      let len = 0;
      for (const c of str) {
        const clen = charLength(c);
        if (len + clen > threshold) {
          break;
        }
        res += c;
        len += clen;
